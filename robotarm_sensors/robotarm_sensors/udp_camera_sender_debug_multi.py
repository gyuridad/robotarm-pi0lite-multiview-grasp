...
class IsaacRgbdUdpSenderMulti(Node):
    def __init__(self):
        super().__init__("isaac_rgbd_udp_sender_multi")
        ...
        self.declare_parameter("dest_targets_csv", "127.0.0.1:9002,127.0.0.1:9003")
        ...
        self.dbg = LDADebugger(
            node_name="isaac_rgbd_udp_sender_multi",
        ...
        self.get_logger().info(
            f"[IsaacRgbdUdpSenderMulti] rgb={self.rgb_topic}, depth={self.depth_topic}, "
        ...
        try:
            ...
            if now - self.last_summary_time >= self.summary_period_sec:
                ...
                self.dbg.emit_periodic_summary(
                    event="udp_frame_send_summary_multi",
            ...
            self.dbg.emit_event(
                "ERROR",
                "on_timer_send_exception_multi",
                ReasonCode.COMMON.EXCEPTION,
                {"error": repr(e)},
            )
            self.get_logger().warn(f"on_timer_send multi error: {e}")
...
## 실행예시
# ros2 run robotarm_sensors udp_camera_sender_debug_multi --ros-args \
#   -p dest_targets_csv:="127.0.0.1:9002"
